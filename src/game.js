import { database, ensureUser, grantDefaults } from './database.js';
import { GAME, SPECIES, SLOT_DEFS } from './config.js';
import { getBase, getDefaultLocation, getItem, getOwnedCandidates } from './catalog.js';

export function getUser(id){ return ensureUser(id); }
export function balanceChange(id, amount, reason){ const u=ensureUser(id); const next=u.balance+amount; if(next<0)return null; database().prepare('UPDATE users SET balance=? WHERE user_id=?').run(next,id); database().prepare('INSERT INTO transactions(user_id,amount,reason,created_at) VALUES(?,?,?,?)').run(id,amount,reason,Date.now()); return next; }
export function rewardMessage(id){ const u=ensureUser(id); const now=Date.now(); if(now-u.last_message_reward<GAME.messageCooldownMs)return; const gain=Math.floor(Math.random()*(GAME.messageRewardMax-GAME.messageRewardMin+1))+GAME.messageRewardMin; database().prepare('UPDATE users SET balance=balance+?,last_message_reward=? WHERE user_id=?').run(gain,now,id); }
export function claimDaily(id){ const u=ensureUser(id); const now=Date.now(); const cd=86400000; if(now-u.last_daily<cd)return {ok:false,remaining:cd-(now-u.last_daily)}; database().prepare('UPDATE users SET balance=balance+?,last_daily=? WHERE user_id=?').run(GAME.dailyReward,now,id); return {ok:true,balance:u.balance+GAME.dailyReward}; }
export function petsOf(id){ return database().prepare('SELECT * FROM pets WHERE user_id=? ORDER BY adopted_at,id').all(id).map(applyDecay); }
...
export function equip(id,petId,itemId){ const p=petById(petId),item=getItem(itemId); if(!p||p.user_id!==id)return {ok:false,reason:'owner'}; if(!item||!owned(id,itemId))return {ok:false,reason:'owned'}; if(item.species!=='all'&&item.species!==p.species_id)return {ok:false,reason:'species'}; const def=(SLOT_DEFS[p.species_id]??[]).find(s=>s.id===item.slot)||(item.slot==='lieu'?{column:'location_id'}:null); if(!def)return {ok:false,reason:'slot'}; database().prepare(`UPDATE pets SET ${def.column}=? WHERE id=?`).run(item.id,p.id); return {ok:true,pet:petById(p.id)}; }
export function unequip(id,petId,slot){ const p=petById(petId); if(!p||p.user_id!==id)return {ok:false}; const def=(SLOT_DEFS[p.species_id]??[]).find(s=>s.id===slot)||(slot==='lieu'?{column:'location_id'}:null); if(!def)return {ok:false}; const fallback=slot==='lieu'?getDefaultLocation()?.id:(slot==='tenue'?getBase(p.species_id)?.id:null); database().prepare(`UPDATE pets SET ${def.column}=? WHERE id=?`).run(fallback,p.id); return {ok:true,pet:petById(p.id)}; }
